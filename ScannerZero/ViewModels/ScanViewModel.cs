...
public sealed partial class ScanViewModel : ViewModelBase, IDisposable
{
    ...
    private void EnsureCamera()
    {
        ...
        if (_camera is not null)
        {
            _camera.CodeDetected += OnCodeDetected;
        }
    }

    private async void OnCodeDetected(object? sender, CodeDetectedEventArgs e)
    {
        if (!_shouldRunCamera)
        {
            return;
        ...
        var parsed = ScanPayloadParser.Parse(e.RawText);
