...
public sealed class DesktopCameraScanService : ICameraScanService
{
    ...
    // and barcode decoding only ever needed luminance anyway.
    ...
    private async Task OnFrameArrivedAsync(PixelBufferScope bufferScope)
    {
        ...
        // Throttle decoding attempts; barcode decoding is comparatively expensive.
        ...
        var result = await BarcodeDecoder.Scan(decodeFrame).ConfigureAwait(false);
    ...
    /// <summary>Returns a smaller copy for fast barcode decoding; returns the original if it's already small enough.</summary>
